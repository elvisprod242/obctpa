...
const tempsReposSchema = z.object({
...
type TempsReposFormValues = z.infer<typeof tempsReposSchema>;
...
function TempsReposForm({
  ...
  const form = useForm<TempsReposFormValues>({
    resolver: zodResolver(tempsReposSchema),
    defaultValues: tempsRepos || {
      analyse_cause: '',
      action_prise: '',
      suivi: '',
    },
  ...
  const onSubmit = async (data: TempsReposFormValues) => {
    if (!activePartner) {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Pause déjeuner" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Aucune" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
export default function TempsReposPage() {
  ...
  const driverAnalysisData = useMemo(() => {
    if (!rapports || !selectedDriverId || !objectifs || !tempsReposData || !invariants || !activePartner) return [];
    ...
    for (const report of driverMonthlyReports) {
      try {
        ...
        weeklyData[weekKey].subTotalSeconds += parseTimeString(report.temps_attente);
