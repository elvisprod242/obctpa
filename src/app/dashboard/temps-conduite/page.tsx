...
const tempsConduiteSchema = z.object({
...
type TempsConduiteFormValues = z.infer<typeof tempsConduiteSchema>;
...
function TempsConduiteForm({
  ...
  const form = useForm<TempsConduiteFormValues>({
    resolver: zodResolver(tempsConduiteSchema),
    defaultValues: tempsConduite || {
      analyse_cause: '',
      action_prise: '',
      suivi: '',
    },
  ...
  const onSubmit = async (data: TempsConduiteFormValues) => {
    if (!activePartner) {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Livraison urgente" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Répartition des tâches" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Temps respecté depuis" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
export default function TempsConduitePage() {
  ...
  const driverAnalysisData = useMemo(() => {
    if (!rapports || !selectedDriverId || !objectifs || !tempsConduiteData || !invariants || !activePartner) return [];
    ...
    for (const report of driverMonthlyReports) {
      try {
        ...
        weeklyData[weekKey].subTotalSeconds += parseTimeString(report.temps_conduite);
